...
#define REPORT_PACKT_MIN_SIZE                   30       // minimum number of packets encoded in the reception report - cant be lower than 10
...
static void IRAM_ATTR start_manually_isr(void* arg){
    if (getCycleFromISR() == HOLD){
        ...
        if (ret == pdPASS) portYIELD_FROM_ISR(HigherPriorityTaskWoken);
    }else if(getCycleFromISR() != SHUTDOWN){
        BaseType_t HigherPriorityTaskWoken, ret;
    ...
    while (xQueueReceive(espnow_event_queue, &evt, portMAX_DELAY) == pdTRUE) {
        ...
        switch(evt.id){
            ...
            case RECV_RECEPREP:
            {
                ...
                ESP_LOGE(TAG4, "Received reception report - signal_length %d data_lenght %d packet_count %d", recv_cb->sig_len, recv_cb->data_len, new_recep_rep.entries);
            ...
            case RECV_ONC_DATA:
            {
                ESP_LOGE(TAG4, "Received encoded packet");
                espnow_event_recv_cb_t *recv_cb = &evt.info.recv_cb;
                ...
                bool minimum_reached = (xPacketPoolGetDecodeCnt() >= REPORT_PACKT_MIN_SIZE);
                bool no_dec_pending  = (getCashedPacketCnt() == 0);
                if (minimum_reached && no_dec_pending){                     // send reception report if minimum decoding count is reached and no pending decodings are in queue 
